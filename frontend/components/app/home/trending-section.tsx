...
export const TrendingSection = () => {
  return (
    <section
      id="trending"
      className="relative w-full md:max-h-110 h-fit overflow-hidden"
    >
      ...
      <div className="wrapper relative z-20 w-full h-full max-md:space-y-8">
        ...
        <div id="carousel" className="w-full h-[calc(100%_-_3rem)] md:h-[calc(100%_-_5rem)]">
          <Carousel className="w-full h-full px-4 relative z-90">
            <CarouselContent className="-ml-1 w-full h-full gap-4 flex items-center">
              {Array.from({ length: 7 }).map((_, index) => (
                <CarouselItem key={index} className="pl-1 basis-1/2 md:basis-1/4 md:max-h-70 h-50">
                  <Card index={index} />
                </CarouselItem>
              ))}
            </CarouselContent>
...
const Card = ({ index }: CardProps) => {
  return (
    <div className="relative p-1 w-full max-w-56 h-full! m-auto bg-slate-600 rounded-lg hover:scale-105 duration-400 transition-all">
      ...
      />
      <div className="absolute -left-3 md:-left-6 bottom-4 flex items-center justify-center h-fit w-fit">
        <span className="relative text-6xl md:text-9xl text-stroke -font-black text-black">
          {index + 1}
        </span>
      </div>
    </div>
